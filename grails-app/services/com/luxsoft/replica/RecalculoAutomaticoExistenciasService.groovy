 ...
 * 
 */
class RecalculoAutomaticoExistenciasService {
	...
	def recalculoAutomatico(){
	
		Date fecha=new Date()
		def mes=periodo.obtenerMes(fecha)
		def year=periodo.obtenerYear(fecha)
		
		def sucur=[3L:'TACUBA',6L:'ANDRADE',9L:'CF5FEBRERO',5L:'BOLIVAR',2L:'CALLE4',11L:'VERTIZ',14L:'SOLIS']
		def suc=['TACUBA':3L,'ANDRADE':6L,'CF5FEBRERO':9L,'BOLIVAR':5L,'CALLE4':2L,'VERTIZ':11L,'SOLIS':14L]
		
		def sucursales=Sucursal.findAllByActivaAndNombreNotEqual('true','OFICINAS')
				sucursales.each{sucursal->
								try {
									
										buscarDiscrepancias(sucursal,suc.get(sucursal.nombre),year,mes)
								} catch (Exception e) {
										e.printStackTrace()
								}
	...
	public  List<Map<String, Object>> buscarDiscrepancias(Sucursal sucursal,Long sucursalId, int year, int mes ){
		...
		String sql="""
				SELECT A.PRODUCTO_ID,A.CLAVE,SUM(CANTIDAD),
					...
					WHERE P.INVENTARIABLE IS TRUE AND S.SUCURSAL_ID='@SUCURSAL' AND year(A.FECHA) ='@YEAR'  AND MONTH(A.FECHA)='@MES' 
		...
		def discrepancias=targetSql.rows(sql)
